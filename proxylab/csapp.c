...
{
    int status;
    if ((status = sigprocmask(how, set, oldset)) < 0)
...
{
    int status; 
    if ((status = sem_init(sem, pshared, value)) < 0)
...
void Rio_writen(int fd, void *usrbuf, size_t n, jmp_buf jmp)
{
